    ...
    return async function (dispatch) {
        ...
        try {
            ...
            if (all) {
                ...
                let {
                    offset = 0, limit = 25, search = null, minPrice = 0, maxPrice = null, brand = null, category = null, sort = null
                } = data;
    ...
    return async function (dispatch) {
        try {
            ...
            return dispatch({
                ...
                payload: {
                    error: true,
                    message: "No se pudo crear la cuenta, revise los datos"
                }
            })
        }
    }
...
export function editCategory(id, name) {
    return async function (dispatch) {
        try {
            ...
            axios.put(`${SERVER}/categories/${id}`, {
                name: name
            })
...
export function editProduct(id, name, price, stock, sold_quantity, condition, image, thumbnail, attribute, categories, brands) {
    return async function (dispatch) {
        try {
            const edProduct = await axios.put(`${SERVER}/products/${id}`, {
                name: name,
                price: price,
                stock: stock,
    ...
    return async function (dispatch) {
        try {
            if (!userId) {
                ...
                const {
                    data
                } = await axios.get(`${SERVER}/users/cart/${userId}`)
    ...
    return async (dispatch) => {
        try {
            if (!userId) {
                ...
                const {
                    data
                } = await axios.delete(`${SERVER}/users/cart/${userId}?idProduct=${idProduct}`)
...
export function changeAmount(products, userId) {
    try {
        return async (dispatch) => {
            if (userId) {
                const qtyProduct = await axios.put(`${SERVER}/users/cart/${userId}`, {
...
export function getOrderProducts(idUser) {
    return async function (dispatch) {
        ...
        const {
            data
        } = await axios.get(`${SERVER}/users/orders/${idUser}`)
        console.log(data)
