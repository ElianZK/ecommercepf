    ...
    try {
      if (!userId) {
        ...
        const { data } = await axios.delete(
          `${SERVER}/users/cart/${userId}?idProduct=${idProduct}`
        );
...
export function clearCart(idUser) {
  return async function (dispatch) {
    if (idUser) await axios.delete(`${SERVER}/users/cart/${idUser}`);
    localStorage.removeItem("cart");
    return dispatch({
      type: CLEAR_CART,
      payload: [],
  ...
  return async function (dispatch) {
    ...
    try {
      let response = await axios.get(
        `http://localhost:3001/users/wishlist/${idUser}`
      );
      dispatch({
        type: GET_WISHLIST,
  ...
  return async function (dispatch) {
    try {
      let response = await axios.post(
        `http://localhost:3001/users/wishlist/${idUser}/${idProduct}`
      );
